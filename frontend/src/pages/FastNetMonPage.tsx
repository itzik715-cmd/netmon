...
function DetectionPanel() {
  ...
  const thresholdRows = (g: any) => [
    { metric: 'Total PPS', enabled: g.ban_for_pps, value: g.threshold_pps, dir: 'incoming' },
    { metric: 'Total Bandwidth', enabled: g.ban_for_bandwidth, value: `${g.threshold_mbps} Mbps`, dir: 'incoming' },
    { metric: 'Total Flows', enabled: g.ban_for_flows, value: g.threshold_flows, dir: 'incoming' },
    { metric: 'TCP PPS', enabled: g.ban_for_tcp_pps, value: g.threshold_tcp_pps, dir: 'incoming' },
    { metric: 'TCP Bandwidth', enabled: g.ban_for_tcp_bandwidth, value: `${g.threshold_tcp_mbps} Mbps`, dir: 'incoming' },
    { metric: 'UDP PPS', enabled: g.ban_for_udp_pps, value: g.threshold_udp_pps, dir: 'incoming' },
    { metric: 'UDP Bandwidth', enabled: g.ban_for_udp_bandwidth, value: `${g.threshold_udp_mbps} Mbps`, dir: 'incoming' },
    { metric: 'ICMP PPS', enabled: g.ban_for_icmp_pps, value: g.threshold_icmp_pps, dir: 'incoming' },
    { metric: 'ICMP Bandwidth', enabled: g.ban_for_icmp_bandwidth, value: `${g.threshold_icmp_mbps} Mbps`, dir: 'incoming' },
    { metric: 'TCP SYN PPS', enabled: g.ban_for_tcp_syn_pps, value: g.threshold_tcp_syn_pps, dir: 'incoming' },
    { metric: 'TCP SYN Bandwidth', enabled: g.ban_for_tcp_syn_bandwidth, value: `${g.threshold_tcp_syn_mbps} Mbps`, dir: 'incoming' },
    { metric: 'IP Fragments PPS', enabled: g.ban_for_ip_fragments_pps, value: g.threshold_ip_fragments_pps, dir: 'incoming' },
  ]
              ...
              <tbody>
                {thresholdRows(g).map((row) => (
                  <tr key={row.metric}>
                    ...
                    <td>{row.enabled ? <span className="tag-green">Yes</span> : <span className="tag-gray">No</span>}</td>
                    <td className="mono">{row.value != null ? String(row.value) : '—'}</td>
                    <td>{row.dir}</td>
                  </tr>
                ))}
              </tbody>
...
function ConfigPanel() {
  ...
  const cfgRow = (label: string, value: any) => (
    <tr key={label}><td><strong>{label}</strong></td><td className="mono">{String(value ?? '—')}</td></tr>
  )
  ...
  return (
    <>
      ...
      <div className="card">
        ...
        <div className="table-wrap">
          <table><tbody>
            {cfgRow('Ban Enabled', c.enable_ban)}
            {cfgRow('Ban Time', `${c.ban_time}s`)}
            {cfgRow('Unban Enabled', c.unban_enabled)}
            {cfgRow('Unban Only If Attack Finished', c.unban_only_if_attack_finished)}
            {cfgRow('Keep Blocked Hosts On Restart', c.keep_blocked_hosts_during_restart)}
            {cfgRow('Process Incoming', c.process_incoming_traffic)}
            {cfgRow('Process Outgoing', c.process_outgoing_traffic)}
            {cfgRow('Process IPv6', c.process_ipv6_traffic)}
          </tbody></table>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="table-wrap">
          <table><tbody>
            {cfgRow('sFlow', c.sflow ? 'Enabled' : 'Disabled')}
            {cfgRow('sFlow Ports', c.sflow_ports?.join(', '))}
            {cfgRow('sFlow Host', c.sflow_host)}
            {cfgRow('NetFlow', c.netflow ? 'Enabled' : 'Disabled')}
            {cfgRow('Speed Calculation Delay', `${c.speed_calculation_delay}s`)}
            {cfgRow('Average Calculation Time', `${c.average_calculation_time}s`)}
          </tbody></table>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="table-wrap">
          <table><tbody>
            {cfgRow('GoBGP Enabled', c.gobgp)}
            {cfgRow('FlowSpec Announces', c.gobgp_flow_spec_announces)}
            {cfgRow('FlowSpec Default Action', c.gobgp_flow_spec_default_action)}
            {cfgRow('FlowSpec Ban Time', `${c.flow_spec_ban_time}s`)}
            {cfgRow('Next Hop (Host)', c.gobgp_next_hop)}
            {cfgRow('Community (Host)', c.gobgp_community_host)}
            {cfgRow('Community (Subnet)', c.gobgp_community_subnet)}
            {cfgRow('Community (Remote)', c.gobgp_community_remote_host)}
          </tbody></table>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="table-wrap">
          <table><tbody>
            {cfgRow('Email Enabled', c.email_notifications_enabled)}
            {cfgRow('SMTP Host', `${c.email_notifications_host}:${c.email_notifications_port}`)}
            {cfgRow('From', c.email_notifications_from)}
            {cfgRow('Recipients', c.email_notifications_recipients?.join(', '))}
            {cfgRow('Telegram', c.telegram_notifications_enabled ? 'Enabled' : 'Disabled')}
            {cfgRow('Slack', c.slack_notifications_enabled ? 'Enabled' : 'Disabled')}
          </tbody></table>
        </div>
      </div>
