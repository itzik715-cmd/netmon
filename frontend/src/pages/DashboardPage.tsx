...
function StatCard({
  ...
  const content = (
    <div className={`stat-card hover:border-slate-600 transition-colors ${to ? 'cursor-pointer' : ''}`}>
  ...
  return (
    <div className="space-y-6">
      <div className="page-header">
        <div>
          <h1>Dashboard</h1>
          <p className="text-sm text-slate-400 mt-0.5">Network overview at a glance</p>
        </div>
      </div>
      ...
      {alertSummary && (
        <div className="grid grid-cols-2 gap-4">
          <div className="card flex items-center gap-4">
            ...
            <div>
              <div className="text-2xl font-bold text-red-400">{alertSummary.critical}</div>
          ...
          <div className="card flex items-center gap-4">
            ...
            <div>
              <div className="text-2xl font-bold text-amber-400">{alertSummary.warning}</div>
              <div className="text-sm text-slate-400">Warning Alerts</div>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/devices" className="text-xs text-blue-400 hover:text-blue-300">
              View all
            </Link>
          </div>
          {downDevices.length === 0 ? (
            <div className="text-center py-8 text-slate-500">
              <Wifi className="h-8 w-8 mx-auto mb-2 text-emerald-600 opacity-50" />
            ...
            <div className="space-y-2">
              {downDevices.slice(0, 5).map((device) => (
                <Link
                  ...
                  className="flex items-center justify-between p-3 rounded-lg bg-dark-200 hover:bg-dark-300 transition-colors"
                >
                  <div>
                    <div className="font-medium text-slate-200 text-sm">{device.hostname}</div>
                    <div className="text-xs text-slate-500">{device.ip_address}</div>
                  </div>
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/alerts" className="text-xs text-blue-400 hover:text-blue-300">
              View all
            </Link>
          </div>
          {!alertEvents || alertEvents.length === 0 ? (
            ...
            <div className="space-y-2">
              {alertEvents.slice(0, 5).map((event: AlertEvent) => (
                <div
                  key={event.id}
                  className="flex items-start justify-between p-3 rounded-lg bg-dark-200"
                >
          ...
          <table>
            ...
            <tbody>
              {(devices as Device[] | undefined)?.map((device) => (
                <tr key={device.id}>
                  <td>
                    <Link to={`/devices/${device.id}`} className="text-blue-400 hover:text-blue-300 font-medium">
                      {device.hostname}
                    </Link>
                    {device.vendor && (
                      <div className="text-xs text-slate-500">{device.vendor} {device.model}</div>
                    )}
                  </td>
                  ...
                  <td className="text-slate-400">{device.location?.name || '—'}</td>
                  <td>{statusBadge(device.status)}</td>
                  <td className="text-slate-400">{device.interface_count ?? 0}</td>
              ...
              {(!devices || (devices as Device[]).length === 0) && (
                <tr>
                  <td colSpan={7} className="text-center text-slate-500 py-8">
                    No devices configured. <Link to="/devices" className="text-blue-400">Add your first device</Link>
                  </td>
                </tr>
              )}
            </tbody>
